...
export class WebContentProcessor {
    ...
    async processPageContent(page, url) {
        try {
            ...
            if (this.options.waitForNavigation) {
                logger.info(`${this.logPrefix} Waiting for possible navigation/redirection...`);
                try {
                    ...
                    await Promise.race([navigationPromise, timeoutPromise])
                        .then(() => {
                        logger.info(`${this.logPrefix} Page navigated/redirected successfully`);
                    })
                        .catch((e) => {
                        // If timeout occurs but page may have already loaded, we can continue
                        logger.warn(`${this.logPrefix} No navigation occurred or navigation timeout: ${e.message}`);
                    });
                }
                catch (navError) {
                    logger.error(`${this.logPrefix} Error waiting for navigation: ${navError.message}`);
            ...
            logger.info(`${this.logPrefix} Successfully retrieved web page content, length: ${html.length}`);
            const processedContent = await this.processContent(html, url);
            // Format the response
            const formattedContent = `Title: ${pageTitle}\nURL: ${url}\nContent:\n\n${processedContent}`;
            return {
    ...
    async processContent(html, url) {
        ...
        if (this.options.extractContent) {
            ...
            if (!article) {
                logger.warn(`${this.logPrefix} Could not extract main content, will use full HTML`);
            }
            else {
        ...
        if (!this.options.returnHtml) {
            ...
            logger.info(`${this.logPrefix} Successfully converted to Markdown, length: ${processedContent.length}`);
        }
        // Truncate if needed
        if (this.options.maxLength > 0 &&
            processedContent.length > this.options.maxLength) {
            logger.info(`${this.logPrefix} Content exceeds maximum length, will truncate to ${this.options.maxLength} characters`);
